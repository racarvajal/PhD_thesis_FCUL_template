...
\begin{table}[tbhp]
    ...
    \begin{tabular}{c S[table-format=2.2+-1.2] S[table-format=2.2+-1.2] S[table-format=2.2+-1.2] S[table-format=2.2+-1.2]} % centred columns (5 columns)
    \toprule
...
Other definitions provided by \texttt{mythesis.cls} include chemical elements. They can be written as \texttt{\textbackslash{}element\{<electrical charge>\}\{<number of nucleons>\}\{<number of protons>\}\{<number of neutrons>\}[<element symbol>]}. For instance:

\begin{description}[nosep]
    \item \element{He}  % Just the element symbol
    \item \element[+2]{He}  % With charge
    \item \element[+2][3]{He}  % With charge and nucleon number
    \item \element[+2][3][4]{He}  % With charge, nucleon number, and proton number
    \item \element[+2][3][4][5]{He}  % All parameters specified
\end{description}
